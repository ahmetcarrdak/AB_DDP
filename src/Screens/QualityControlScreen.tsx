...
const QualityControlScreen = memo(() => {
  ...
  const columns = [
    { title: "Ürün No", data: "productId" },
    { title: "Test Türü", data: "testType" },
    { title: "Test Sonucu", data: "testResult" },
    { title: "Test Tarihi", data: "testDate" },
    { title: "Test Eden", data: "testedBy" },
    { title: "Yorumlar", data: "comments" },
  ];
      ...
      <Spin spinning={loading} tip="Veriler yükleniyor">
        <div className="screen-body">
          ...
          <div className="table-head">
            ...
            <button
              onClick={() => {
                ...
                const tableRows = paginatedData.map((row) =>
                  columns.map((col) =>
                    col.data === "testDate"
                      ? formatDate(row[col.data])
                      : row[col.data]
                  )
                );
                //@ts-ignore
                doc.autoTable(tableColumn, tableRows);
                doc.save("kalite-kontrol-kayitlari.pdf");
              }}
            ...
            <Link to="/quality-control-create" className="table-action-button">
              <IoIosAdd />
              <span style={{ paddingLeft: 10 }}>Kalite Kontrol Kaydı Ekle</span>
            </Link>
          </div>
            ...
            <tbody>
              {paginatedData.length > 0 ? (
                paginatedData.map((record, index) => (
                  <tr
                    key={record.qualityControlRecordId}
